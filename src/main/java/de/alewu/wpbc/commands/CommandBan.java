...
public class CommandBan extends PunishmentCommand {
    ...
    void sendHelp(CommandSender pp, MlPunishments ml, int page) {
        pp.sendMessage(tc(Constants.PREFIX + ml.banWrongUsage()));
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (args.length == 0) {
                ...
                sendTemplates(pp, PunishmentType.BAN, null);
                return;
            }
            ...
            if (args.length == 2) {
                ...
                if (punishmentCache.getActiveBan(targetUser.getId()) != null) {
                    pp.sendMessage(tc(Constants.PREFIX + String.format(ml.banAlreadyBanned(), targetUser.getUsername())));
                    return;
                }
                PunishmentTemplate template = punishmentTemplateCache.find(templateName, PunishmentType.BAN).orElse(null);
                if (template == null) {
                    pp.sendMessage(tc(Constants.PREFIX + String.format(ml.punishmentUnknownTemplate(), templateName)));
                    return;
                }
                Punishment punishment = Punishment.forBan(targetUser.getId(),
                ...
                if (targetPlayer != null) {
                    PunishmentCommunicationHelper.help(targetPlayer, MlPunishments.get(targetUser.getLanguage().toString()))
                        .banPlayer(template.getTemplateReason().replaceAll("_", " "), punishment.getPunishmentDuration());
                }
                ProxyServer.getInstance().getPlayers().forEach(x -> {
                    ...
                    if (x.hasPermission("wp.punishments.broadcast") && (x.getUniqueId().equals(pp.getUniqueId()) || receiveTeamNotifications[0])) {
                        ...
                        x.sendMessage(tc(String.format(ml.banBroadcast(), Constants.PREFIX,
                ...
                if (args.length == 1) {
                    ...
                    sendTemplates(pp, PunishmentType.BAN, targetUser.getId());
                }
            }
        }
    }
