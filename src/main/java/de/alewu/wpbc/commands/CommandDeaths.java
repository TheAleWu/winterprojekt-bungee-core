...
public class CommandDeaths extends Command {

    private final UserCache userCache;

    public CommandDeaths() {
        super("deaths", null);
    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (user.getDeathCount() == 1) {
                pp.sendMessage(tc(Constants.PREFIX + String.format(ml.deathsMessageSingle(), user.getDeathCount())));
            } else {
                pp.sendMessage(tc(Constants.PREFIX + String.format(ml.deathsMessageMultiple(), user.getDeathCount())));
            }
        }
    }
}
