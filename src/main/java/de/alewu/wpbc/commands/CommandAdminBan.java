...
public class CommandAdminBan extends PunishmentCommand {

    public CommandAdminBan() {
        super("adminban", "wp.adminban", "ab");
    }

    @Override
    void sendHelp(CommandSender pp, MlPunishments ml, int page) {
        pp.sendMessage(tc(Constants.PREFIX + ml.adminBanWrongUsage()));
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        if (sender instanceof ProxiedPlayer) {
            ...
            if (args.length >= 3) {
                ...
                Punishment punishment = Punishment.forAdminBan(targetUser.getId(),
                ...
                if (targetPlayer != null) {
                    PunishmentCommunicationHelper.help(targetPlayer, MlPunishments.get(targetUser.getLanguage().toString()))
                        .banPlayer(reason.toString().replaceAll("_", " "), punishment.getPunishmentDuration());
                }
