...
public class CommandFarmwelt extends Command {

    private final UserCache userCache;

    public CommandFarmwelt() {
        super("farmwelt", null, "fw");
            ...
            if (args.length == 0) {
                ...
                try {
                    ...
                    if (server == 1) {
                        ...
                        pp.sendMessage(tc(Constants.PREFIX + "§c/fw [1/2]"));
                    }
    ...
    private void sendToServer(CommandSender sender, ProxiedPlayer pp, MlGeneral ml, boolean random, String farmServer) {
        ...
        if (changedServer) {
            ProxyServer.getInstance().getScheduler().schedule(WinterProjectBungeeCore.INSTANCE, () ->
                StaticMethodCollection.sendToServer(serverInfo, "TeleportToFarmSpawn", ((ProxiedPlayer) sender).getUniqueId().toString()), 1, TimeUnit.SECONDS);
        } else {
            StaticMethodCollection.sendToServer(serverInfo, "TeleportToFarmSpawn", ((ProxiedPlayer) sender).getUniqueId().toString());
            pp.sendMessage(tc(Constants.PREFIX + ml.normalFarm()));
        }
    }
}
